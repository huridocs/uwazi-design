...
function YouTubeMedia({ media, id }: { media: MediaValue; id: string }) {
  ...
  const firstChapter = media.chapters[0]?.label;
  ...
  return (
    ...
    >
      <div data-part="stage" className="relative w-full aspect-video overflow-hidden rounded-md bg-vellum">
        {loaded ? (
          <iframe
            key={loadNonce}
            ref={frameRef}
          ...
          >
            <span
              aria-hidden
              className="flex items-center justify-center w-14 h-14 rounded-full bg-ink text-paper shadow-sm
                group-hover:scale-105 transition-transform"
            >
              <Play size={22} className="translate-x-0.5" />
            </span>
            <span aria-hidden className="px-4 text-center text-meta text-ink-tertiary">
              {firstChapter ? `${media.chapters.length} chapters · ` : ""}Loads from YouTube when you press play
            </span>
          </button>
        )}
      </div>
