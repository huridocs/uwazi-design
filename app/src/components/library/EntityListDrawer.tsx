...
/** A list of entities in the Library drawer: the multi-selection
...
export function EntityListDrawer({
  ...
  icon: ReactNode;
  title: string;
  /** The "N entities" figure, when it isn't `ids.length`. */
  count?: number;
  ids: readonly string[];
  onClose: () => void;
  closeLabel: string;
  ...
  query: string;
  /** The selection drawer's per-row "Remove from selection"; `can` says
   *  whether a row shows the X. Absent: no remove slot at all. */
