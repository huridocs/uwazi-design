 ...
 *  loaded ones — and only those. Selecting the rest of a larger result set is
 *  the footer readout's own offer, so this box never reaches past what the
 *  reader can see.
 *
 *  Its tooltip carries the gestures, because a checkbox is not where anyone
 *  expects to learn them. */
...
export function SelectAllBox({ loadedIds }: { loadedIds: readonly string[] }) {
    ...
    <span title={text} className="inline-flex items-center">
      <input
        ...
        onClick={(e) => {
          ...
          // Clears only what it can see: the loaded ids. Clear, in the bar, is
          // the one that ends a whole selection.
