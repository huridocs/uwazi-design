...
export function DocumentRendition({ format }: { format: DocumentFormat }) {
  ...
  if (format === "text") {
    return (
      <div
        data-component="DocumentRendition"
        data-variant="text"
        className="absolute inset-0 overflow-auto bg-paper"
      >
        <pre
          data-part="body"
          dir={rtl ? "rtl" : "ltr"}
  ...
  return (
    <div
      ...
      className="absolute inset-0 overflow-auto bg-paper"
    >
      <article data-part="body" dir={rtl ? "rtl" : "ltr"} className="mx-auto max-w-[44rem] px-6 body-top pb-8">
        {rendition.html.map((block, i) => {
          switch (block.type) {
            ...
            case "num":
              return (
                <p key={i} data-part="paragraph" className="flex gap-2 text-sm text-ink-secondary leading-relaxed mb-3">
                  <span data-part="paragraph-number" className="font-semibold text-ink shrink-0 tabular-nums">{block.n}</span>
                  <span>{block.text}</span>
                </p>
              );
            default:
              return (
                <p key={i} data-part="paragraph" className="text-sm text-ink-secondary leading-relaxed mb-3">
                  {block.text}
                </p>
              );
          }
